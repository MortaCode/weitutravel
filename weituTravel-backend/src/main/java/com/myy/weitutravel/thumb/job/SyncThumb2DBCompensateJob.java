...
@Slf4j
@Component
public class SyncThumb2DBCompensateJob {
    ...
    public void run() {
        log.info("开始补偿数据");
        Set<String> thumbKeys = redisTemplate.keys(RedisUtil.tempThumbKey("") + "*");
        Set<String> needHandleDataSet = new HashSet<>();
        thumbKeys.stream().filter(ObjUtil::isNotNull).forEach(thumbKey -> needHandleDataSet.add(thumbKey.replace(Constants.TEMP_THUMB_KEY_PREFIX.formatted(""), "")));
